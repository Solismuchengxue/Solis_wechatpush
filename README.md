...
安装前需要先在企业微信后台拿到三个凭证：`corp_id`、`corp_secret`、`agent_id`。
...
服务端主动推送**不需要**配置可信 IP / 可信域名，完成上面 1–4 步拿到三个参数即可使用。

仅当你在「应用 → 企业可信IP」里**手动开启了 IP 白名单**、且推送报 `ErrCode 60020` 时，才需要：
1. 进入企业微信后台 → **应用管理** → 选择应用 → **企业可信IP**
2. 把打印机上位机的**出口公网 IP** 加入白名单（或直接关闭该白名单）

## 📲 第二步：在个人微信中接收通知（推荐）

企业微信后台提供「**微信插件**」功能：成员用手机微信扫码关注后，**无需安装企业微信客户端**，打印机的推送消息会直接出现在个人微信中，和公众号消息体验一致。

1. 登录企业微信管理后台 → 左侧菜单「**微信插件**」
2. 找到「**邀请关注**」→ 点「获取更多尺寸的二维码」
3. 用**手机微信**（不是企业微信）扫码，点「关注」

关注后无需改任何配置，下次打印机推送即可在微信中收到。
...
在 `moonraker.conf` 中填入第一步获取的三个凭证：
...
> ✅ **关于"可信 IP / 可信域名"：** 本插件通过服务端 API（`gettoken` / `message/send` / `media/upload`）**主动推送**消息，**无需配置可信域名或可信 IP**。
>
> 可信域名 / 可信 IP 只在「网页授权登录（OAuth2）、JS-SDK、应用主页」等"在企业微信内打开网页"的场景才需要，本插件不涉及。
>
> 仅当你在「应用 → 企业可信IP」里**手动开启过 IP 白名单**，推送时才可能出现 `ErrCode 60020`：把上位机的**出口公网 IP** 加入白名单，或直接关闭该白名单即可。
